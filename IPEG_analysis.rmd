...
setwd('C:/Users/dpen466/Google Drive/Phd (1)/Experiment/Studies/Data/IPE/IPEG/IPEG/IPEG_analysis/') #to change
...
step(zcp_model<-lmer(Help~Conditions +HT_sex_c+HT_age_c+(dummy_img||participant)+(dummy_img||Story_ID) +(dummy_img||HTFace_ID), data=IPEG.df))# only retain dummy_img in participant
# Reduced model
zcp_model_rm<-lmer(Help~Conditions +HT_sex_c+HT_age_c+(dummy_img||participant)+(1|Story_ID) +(1|HTFace_ID), data=IPEG.df)
# Include correlation parameters to retained random slopes
final_model<- lmer(Help~Conditions+HT_sex_c+HT_age_c+(dummy_img|participant)+(1|Story_ID) +(1|HTFace_ID), data=IPEG.df)
# Correlation parameters significantly improve model fit
anova(zcp_model_rm,final_model, refit=F)
# final_model identified as the parsimonious model

# F-test of fixed effect with Kenward-Roger approximation of degress of freedom found significant condition effect
anova(final_model, test='kenward-roger')

#extract means for each conditions
(posthoc<- emmeans(final_model, pairwise~Conditions, adjust= 'none'))
confint(posthoc)
```
#### Plots
```{r}
...
```{r}
# R2m = marginal R2 (R2 of fixed effects), R2c= conditional R2 (R2 of fixed + random effects)
...
#### Coding Match_Condition variable
```{r}
...
```{r}
## Zero-correlation parameter model and determine significance of varying slopes
step(zcp_genmodel<- lmer(Help~Match_condition+HT_sex_c+HT_age_c+(backdiff_2v1+backdiff_3v2+backdiff_4v3+backdiff_5v4||participant)  +(backdiff_2v1+backdiff_3v2+backdiff_4v3+backdiff_5v4||HTFace_ID)+(backdiff_2v1+backdiff_3v2+backdiff_4v3+backdiff_5v4||Story_ID), data=IPEG.df, contrast= list (Match_condition= MASS::contr.sdif))) # rertain only backdiff_2v1 and backdiff_5v4 for participant
# Reduce model
...
## Model failed to converge, refit the model with better starting values (i.e. end values of previous fits) for REML. Model converged after 3 refits
rm_genmodel<- refit_LMM(rm_genmodel)
## Final model
...
anova(rm_genmodel, test='kenward-roger')
## Results
summary(rm_genmodel)
```
...
test<-(abs(posterior_gen[, "b_Match_conditionSMAM"]-posterior_gen[, "b_Match_conditionSPAM"])<0.1) &
  (abs(posterior_gen[, "b_Match_conditionSMAP"]-posterior_gen[, "b_Match_conditionSPAM"])<0.1) &
  (abs(posterior_gen[, "b_Match_conditionSPAM"]-posterior_gen[, "b_Match_conditionSPAP"])<0.1)
(test_BF<-restricted_BF(test, 4e4, 5))

restrictedmodel2_BF/test_BF
restrictedmodel1_BF/test_BF
```
